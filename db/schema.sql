-- Hawt E-Commerce — Scalable Schema
-- Run: psql -U postgres -d hawt -f db/schema.sql
...
-- One row per size — this is the unit inventory tracks and locks
...
-- Tracks per-user coupon consumption; order_id FK wired below after orders table
...
-- Denormalized: captures product name/size/price at the moment of purchase
...
-- Bridges orders ↔ payment offers; UNIQUE prevents payment-offer replay attacks
...
-- ─── BACK-REFERENCE: coupon_usage.order_id ────────────────────────────────────
...
-- Async event queue — worker polls PENDING rows, publishes Email/SMS/fulfillment jobs
CREATE TABLE outbox (
  id             UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Partial indexes scoped to active products — smaller, faster, match query WHERE clauses
CREATE INDEX idx_products_active_cat  ON products(category)                        WHERE is_active = TRUE;
CREATE INDEX idx_products_flags       ON products(is_new_in, is_on_sale, is_clearance) WHERE is_active = TRUE;
